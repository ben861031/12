window.App = {
        ...
        if (sems.length === 0) {
          ...
          historyList.innerHTML = sems.map(sem => `
            <div class="flex justify-between items-center py-1 border-b border-slate-200/60 last:border-0">
              <span class="font-bold text-slate-700">\u2705 ${sem} \u5b78\u671f</span>
              <span class="text-slate-500">${history[sem]}</span>
            </div>
          `).join('');
        }
      }
    ...
    if (tbody) {
      tbody.innerHTML = mismatches.map(m => `
        <tr class="border-b border-rose-100 hover:bg-rose-50/50">
          <td class="text-center font-mono font-bold text-slate-500">${m.rowNum}</td>
          <td class="font-mono font-bold text-slate-900">${m.studentId}</td>
          <td>${m.excelClass}</td>
          <td class="font-bold text-indigo-700">${m.excelName}</td>
          <td class="font-bold ${m.type === 'NAME_MISMATCH' ? 'text-rose-600' : 'text-slate-400'}">${m.systemName}</td>
    ...
    if (tbody) {
      tbody.innerHTML = scoreConflicts.map(c => `
        <tr class="border-b border-slate-100 hover:bg-slate-50">
          <td class="px-4 py-3 font-mono font-bold">${c.studentId}</td>
          <td class="px-4 py-3 font-bold text-slate-800">${c.name}</td>
          <td class="px-4 py-3 text-center font-mono font-bold text-slate-600">${c.semester}</td>
    ...
    if (tbody) {
      tbody.innerHTML = logicConflicts.map(c => `
        <tr class="border-b border-slate-200 hover:bg-rose-50/50">
          <td class="font-mono font-bold text-slate-900">${c.studentId}</td>
          <td class="font-bold text-slate-800">${c.name}</td>
          <td class="text-center font-bold text-emerald-600">${c.actualPasses} \u6b21</td>
          <td class="text-center font-bold text-rose-600">${c.excelStatus}</td>
        </tr>
      `).join('');
    }
  ...
  executeImport(scanResult, semester, resolution, logicResolution) {
    ...
    this.showToast(`\u9ad4\u9069\u80fd\u6210\u7e3e\u532f\u5165\u6210\u529f\uff01\u5171\u66f4\u65b0 ${res.updatedCount} \u4f4d\u5b78\u751f\u72c0\u614b (\u65b0\u589e ${res.addedStudentCount} \u4eba)`, 'success');
    window.AdminPortal.renderCurrentView();
  },
  clearAllData() {
  ...
  saveFirebaseConfig() {
    ...
    if (!apiKey || !projectId) {
      ...
      this.showToast('\u5df2\u6e05\u7a7a Firebase \u8a2d\u5b9a\uff0c\u5207\u56de LocalStorage \u6a21\u5f0f', 'info');
    } else {
      const config = { apiKey, projectId };
      const success = window.FitnessFirebase.saveConfig(config);
      if (success) {
        this.showToast('\u6210\u529f\u9023\u7dda Firebase \u96f2\u7aef\u8cc7\u6599\u5eab\uff01', 'success');
      } else {
